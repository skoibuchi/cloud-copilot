...
from app.utils.embedding import Embedding


class RAGToolClass:
    def __init__(self, vectorstore_class="chroma", user="default"):
        """
        Initialize RAG tool with the specified vectorstore class.
        RAGツールを指定されたベクターストアで初期化します。
        """
        self.emb = Embedding(vectorstore_class=vectorstore_class, user=user)
        self.emb.load_store()
...
def create_rag_tool_instance(vectorstore_class="chroma", user="default") -> RAGToolClass:
    """
    ...
    """
    return RAGToolClass(vectorstore_class=vectorstore_class, user=user)
