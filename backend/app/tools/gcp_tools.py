...
class DeleteFileInput(FileOperationInput):
    blob_name: str = Field(..., description="Blob name in bucket")
...
async def _list_files(user_id: int, bucket_name: str, account_name: str) -> list[str]:
    """
    List all blobs in the specified GCS bucket for this user.
    指定バケット内のオブジェクト一覧を返す
    """
    _, storage_client, _, _, _ = get_gcp_clients(user_id, account_name)
    bucket = storage_client.bucket(bucket_name)
...
async def _delete_file(user_id: int, bucket_name: str, blob_name: str, account_name: str) -> str:
    """
    Delete a blob from the specified GCS bucket for this user.
    指定バケットからオブジェクトを削除
    """
    _, storage_client, _, _, _ = get_gcp_clients(user_id, account_name)
    bucket = storage_client.bucket(bucket_name)
